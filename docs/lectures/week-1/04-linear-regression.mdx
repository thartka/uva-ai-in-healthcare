---
sidebar_position: 4
title: "Lecture 1.1.4 – Linear Regression"
---
...
# Lecture 1.1.4 – Linear Regression
...
This isn't just about comfort—overcrowding in the ED is associated with:
...
## NEDOCS at UVA: A Real-World Implementation
...
- **β₁** = the slope (how much y changes for each unit increase in x)
...
- The effect of each variable is **additive**—you can add up the contributions
...
So far, we've talked about what linear regression does—it finds a line that fits the data. But what do we do when the data cannot be connected with a straight line? 
...
<img 
  src="/img/multivariable-regression-example.png" 
  alt="Multivariable regression example" 
  style={{
    width: '100%',
    maxWidth: '800px',
    height: 'auto',
...
:::success ✋ Hands-on Activity: Bi-variate vs Multi-variable Regression
...
While simple regression has: **y = β₀ + β₁x + ε**
...
- **x₁, x₂, x₃, ...** = different predictor variables
- **β₁, β₂, β₃, ...** = coefficients for each predictor
...
**Coefficients** (the β values) tell us how much the outcome changes when a predictor changes by one unit, **holding all other variables constant**.

### NEDOCS Formula Breakdown

The NEDOCS formula:

**NEDOCS = 85.8 × (Total ED Patients / ED Beds) + 600 × (Admits / Hospital Beds) + 5.64 × (Wait Time to Bed) + 0.93 × (Longest Admit Wait) + 13.4 × (Ventilator Patients)**

#### Interaction Terms

The NEDOCS formula shown here uses **additive terms**—each variable contributes independently to the score. In some cases, we might want to model **interactions**, where the effect of one variable depends on the value of another. For example, we could add an interaction term like (Total Patients / ED Beds) × (Admits / Hospital Beds) to capture the combined effect when both occupancy and hospital capacity are high. Interaction terms allow us to model more complex relationships where variables work together rather than simply adding their individual effects.

### Interpreting Coefficients

Let's break down what each coefficient means:

#### Coefficient: 600 for (Admits / Hospital Beds)
- **Meaning**: Hospital capacity constraints have a major impact on ED crowding
- **Clinical interpretation**: When the hospital is full, admitted patients board in the ED, significantly increasing crowding
- **Example**: If admits/hospital beds ratio is 0.1 (10%), this contributes 60 points to the NEDOCS score
...
#### Coefficient: 13.4 for (Ventilator Patients)
- **Meaning**: Each ventilator patient adds 13.4 points to the NEDOCS score
- **Clinical interpretation**: Ventilator patients require intensive resources and signal high acuity
- **Example**: 2 ventilator patients contribute 26.8 points
...
#### Coefficient: 0.93 for (Longest Admit Wait)
- **Meaning**: Each hour of the longest admit wait adds 0.93 points
- **Clinical interpretation**: The longest wait highlights extreme boarding situations
- **Example**: A 6-hour longest wait contributes 5.6 points
...
3. **Coefficients quantify contributions**—each coefficient tells you how much that variable affects the outcome while holding all other variables constant, which helps prioritize interventions and understand which factors matter most

4. **Linear regression has strengths** (interpretability, prediction, quantifying relationships) and **limitations** (assumes linearity, sensitive to outliers, may oversimplify complex scenarios)—know when it's appropriate and when to consider alternatives
...
## References
- Fisher, R. A. (1922). The goodness of fit of regression formulæ, and the distribution of regression coefficients. Journal of the Royal Statistical Society, 85(4), 597-612
